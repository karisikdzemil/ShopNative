...
export default function CartItem ( {id} ) {
    ...
    const cartTargetItem = cartItems.cartItems.find((el) => el.productId === id);
    const dispatch = useDispatch();

    const itemHandler = (operation) => {
        if(operation === "-") {
            dispatch(removeItem({productId: cartTargetItem.productId}));
        }
        if(operation === "+"){
            dispatch(addItem({
            productId: cartTargetItem.productId,
            quantity: 1,
          }))
        }
    }
