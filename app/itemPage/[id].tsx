...
export default function ItemPage() {
  ...
  const product: Product | undefined = !loading
  ...
  const addToCartHandler = async () => {
    ...
    try {
      ...
      const cartItemRef = doc(db, "users", userId, "cartItems", String(product.id));
  ...
  return (
    <View style={{ backgroundColor: "#121212", flex: 1 }}>
      <ScrollView className="w-full pt-14 pb-14" contentContainerStyle={{ paddingBottom: 40, paddingTop: 20 }}>
        <Image source={{ uri: product.image }} resizeMode="cover" className="w-full h-[500px] rounded-xl" />

        <View className="p-5">
          <Image source={{ uri: product.image }} resizeMode="cover" className="w-16 h-16 rounded-xl mt-5" />
          <Text className="text-white font-bold text-3xl mt-5">{product.title}</Text>
          <Text className="text-xl text-gray-400 mt-2">{product.category}</Text>
          <Text className="mt-2 text-gray-400 flex-row items-center">
            <FontAwesome name="star" size={17} color="yellow" /> {product.rating.rate} ({product.rating.count} reviews)
          </Text>

          <View className="flex-row gap-3">
            <Text className="text-4xl text-white font-bold mt-5">${product.price}</Text>
